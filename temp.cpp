...
//     }
//     int cnt = 0;
//     if(curr%2==0 && curr+1<=lim) {
//         cnt += f(curr+1, rem-1);
//     }
//     if(curr*2 <= lim) cnt += f(curr*2, rem-1);
//     return cnt+1;
// }
int dp[35][2][1000];
string s = "";
int f(int i, bool flg, int curr, const int k) {
    if(curr > k+1) return 0;
    if(i == s.size()) {
        return 1;
    }

    if(dp[i][flg][curr] != -1) return dp[i][flg][curr];
...
void solve()
{
    ...
    cin >> n >> k;
    // lim = n;
    // // lim = min(lim, (int)(1 << (k-1)));
    // int ans = 0;
    // ans += max(0LL, (int)(n - pow(2, k-1)));
