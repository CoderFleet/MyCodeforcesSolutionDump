...
int rec(const string& lim, int idx, int nZero, bool smaller) {
    if(nZero > 3) return 0;
    int sz = lim.size();
    if(idx==sz) return 1;

    int res = rec(lim, idx+1, nZero, smaller?smaller:(lim[idx] != '0'));
    if(smaller) {
        res += 9*rec(lim, idx+1, nZero+1, 1);
    } else {
        int lsr = lim[idx] - '0' - 1;
        if(lsr > 0) res += lsr * rec(lim, idx+1, nZero+1, 1);
        if(lim[idx] != '0') res += rec(lim, idx+1, nZero+1, 0);
    }
