...
void solve(){
    ...
    string ba = "";
    ba += '1';
    for(int i=0; i<la-1; ++i) ba += '0';
    int baseA = stoi(ba);

    int bestA = pow(10, la)-1;

    int baseC = 1;
    for(int i=0; i<lc-1; ++i) baseC *= 10;

    int baseB = pow(10, lb-1);
    baseB = max(baseC-baseA, baseB);
    cout << baseA << " "<< baseB << " " << baseC << endl;
    //  cout << baseB << endl;

    int bestB = pow(10, lb)-1;
    int bestC = pow(10, lc)-1;
    // cout << bestC - baseA - baseB+1 << endl;

    int tms = min(bestC - baseA - baseB+1, bestB-baseB+1);
    // cout << tms << endl;
