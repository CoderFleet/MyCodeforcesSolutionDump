...
T __gcdCp(T m, T n) {
    while (n != 0) {
        T temp = m % n; // Calculate the remainder
        m = n;           // Assign n to m
        n = temp;        // Assign the remainder to n
    }
    return m;            // When n becomes 0, m holds the GCD
}
...
void solve(){
    ...
    for(int i=0; i<n; ++i) {
        int temp; cin >> temp;
        mpp[temp].push_back(i+1);
    }
    int ans = -1;
    for(int i=1; i<1001; ++i) {
        for(auto& ind: pairs[i]) {
            if(!mpp[i].empty() && !mpp[ind].empty()) ans = max(ans, mpp[i].back()+mpp[indw].back());
        }
    }
 ...
 cin.tie(NULL);
    for(int i=1; i<1001; ++i) {
        for(int j=1; j<1001; ++j) {
            if(__gcdCp(i, j) == 1) pairs[i].push_back(j);
        }
    }
    int T = 1;
    cin >> T;
