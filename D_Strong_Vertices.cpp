...
void solve(){
    ...
    vector<pair<int, int>> fct(n);
    fr(i, n) {
        fct[i].first = a[i] - b[i];
        fct[i].second = i;
    }

    sort(all(fct), [&](const pair<int, int>& p1, const pair<int, int>& p2) {
        if(p1.first != p2.first) return p1.first > p2.first;
        else return p1.second < p2.second;
    });
    vi ans;
    ans.pb(fct[0].second+1);
    rep(i, 1, n-1) {
        if(fct[i].first == fct[0].first) ans.pb(fct[i].second+1);
        else break;
    }
    cout << ans.size() << endl;
    cout << ans << endl;
