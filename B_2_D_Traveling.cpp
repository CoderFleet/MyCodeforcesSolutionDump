...
void solve(){
    ...
    int def = abs(x[a-1] - x[b-1]) + abs(y[a-1] - y[b-1]);

    int fst = 1e18, scnd = 1e18;
    for(int i=0; i<k; ++i) {
        fst = min(fst, abs(x[a-1] - x[i]) + abs(y[a-1] - y[i]));
        scnd = min(scnd, abs(x[b-1] - x[i]) + abs(y[b-1] - y[i]));
    }

    cout << min(fst+scnd, def) << endl;
}
