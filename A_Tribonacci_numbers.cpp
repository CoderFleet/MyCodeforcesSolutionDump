...
// void multiply(vvi& A, vvi& B, vvi& C) {
//     for(int i = 0; i < 3; i++)
//         for(int j = 0; j < 3; j++)
//             C[i][j] = ((A[i][0]*B[0][j])%MOD + (A[i][1]*B[1][j])%MOD + (A[i][2]*B[2][j])%MOD)%MOD;
// }
...
void multiply(vvi &A, vvi &B, vvi &C) {
    ...
    for(int i=0;i<3;i++)
        for(int j=0;j<3;j++)
            temp[i][j] = ((A[i][0]*B[0][j])%MOD + (A[i][1]*B[1][j])%MOD + (A[i][2]*B[2][j])%MOD)%MOD;
    C = temp;
}
vvi pw[61];
void solve(){
    ...
    n -= 2;
    // vvi mat = {{1, 1, 1}, {1, 0, 0}, {0, 1, 0}};
    vvi res = {{1,0,0},{0,1,0},{0,0,1}};

    // while(n) {
    //     if(n%2) multiply(res, mat, res);
...
int32_t main()
{
 ...
 cin.tie(NULL);
    pw[0] = {{1, 1, 1}, {1, 0, 0}, {0, 1, 0}};;
    for(int i=1;i<61;i++) {
        multiply(pw[i-1], pw[i-1], pw[i]);
    }
    int T = 1;
    cin >> T;
    while (T--)
