...
void dfs(int curr, int par, vvi& dp, unordered_map<int, vector<int>>& mpp, vvi& cons) {
    for(auto& it: mpp[curr]) {
        if(it == par) continue;
        dfs(it, curr, dp, mpp, cons);

        dp[curr][0] += max(abs(cons[curr][0]-cons[it][0]) + dp[it][0], abs(cons[curr][0]-cons[it][1]) + dp[it][1]);
        dp[curr][1] += max(abs(cons[curr][1]-cons[it][0]) + dp[it][0], abs(cons[curr][1]-cons[it][1]) + dp[it][1]);
    }
}
