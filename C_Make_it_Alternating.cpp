...
long long modfact(long long n, long long mod = MOD) {
    long long res = 1;
...
void solve(){
    ...
    for(int i=1; i<n; ++i) {
        if(s[i] == s[i-1]) {
            ...
            if(prev) ++loc;
            cnt += prev;
            // int val = modfact(prev+1, MOD);
            modmul(ways, (prev+1)%MOD);
            prev = 0;
        }
    }
    if(prev) ++loc;
    ...
    modmul(ways, (prev+1)%MOD);
    prev = 0;
    // if(loc) modmul(ways, loc);
    // modmul()
    cout << cnt << " " << (ways * modfact(cnt, MOD)) % MOD << endl;
}
