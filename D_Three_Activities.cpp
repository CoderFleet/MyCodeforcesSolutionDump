    ...
    for(int d=0; d<n; ++d) {
        for(int i: {0, 1}) {
            for(int j: {0, 1}) {
                for(int k: {0, 1}) {
                    if(i == 0) {
                        dp[d+1][1][j][k] = max(dp[d+1][1][j][k], dp[d][0][j][k] + a[d]);
                    }
                    if(j==0) {
                        dp[d+1][i][1][k] = max(dp[d+1][i][1][k], dp[d][i][0][k] + b[d]);
                    }
                    if(k==0) {
                        dp[d+1][i][j][1] = max(dp[d+1][i][j][1], dp[d][i][j][0] + c[d]);
                    }
                    dp[d+1][i][j][k] = max(dp[d+1][i][j][k], dp[d][i][j][k]);
                }
            }
        }
    }    
