...
bool dfs(int node, vvi& g, vi& p, vi& h, vi& dp, vi& gud, int par = -1) {
    int hsum = h[node];
    bool inval = true;
    int cgud = 0;
    dp[node] = p[node];
    for(auto it: g[node]) {
        if(it == par) continue;
        inval = inval & dfs(it, g, p, h, dp, gud, node);
        hsum += h[it];
        dp[node] += dp[it];   
        cgud += gud[it];
    }

    if(abs(h[node]) > dp[node]) return false;
    if((h[node] + dp[node])&1) return false;
    int val = (h[node] + dp[node])/2;
    if(val < cgud) return false;
    gud[node] = val;
    // if(hsum > h[node]) return false;
