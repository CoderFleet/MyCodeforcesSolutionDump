...
bool isValid(int node, vi& a, vvi& g, int cvd) {
    // Kuch Nahi
}
...
void solve(){
    ...
    vi a(n); cin >> a;
    unordered_map<int, int> ntoind;
    fr(i, n) {
        ntoind[a[i]] = i;
    }
    // cout << ntoind << endl;
    // for(auto ls: g) {
    //     sort(all(ls), [&](const auto& e1, const auto& e2){
    //         return ntoind[e1]<ntoind[e2];
    //     });
    // }
    ...
    while(!q.empty()) {
        ...
        for(auto it: g[p.first]) {
            if(it == p.second || it == p.first) continue;
            q.push({it, p.first});
        }
    }
    // cout << out << endl;
