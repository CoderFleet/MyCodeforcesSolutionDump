...
void dfs(int ind, int par, int lvl, vi& level, vvi& g, vi& dp) {
    level[ind] = lvl;
    int cnt = 0;

    for(auto it: g[ind]) {
        if(it == par) continue;
        dfs(it, ind, lvl+1, level, g, dp);
        cnt += dp[it];
    }
...
void solve(){
    ...
    vi level(n+1);
    vi dp(n+1, 0);
    dfs(1, -1, 0, level, g, dp);


    vector<int> dist;
    rep(i, 1, n) {
        dist.pb(level[i] - dp[i] + 1);
    }
    ...
    //     // sort(all(lvl[i]), [&](auto v1, auto v2) {
    //     //     return g[v1].size() < g[v2].size();
    //     // });

    //     int rem = min(k, (int)lvl[i].size());
    //     if(rem < lvl[i].size()) {
    //         sort(all(lvl[i]), [&](auto v1, auto v2) {
    //             return g[v1].size() < g[v2].size();
    //         });
