...
void solve(){
    ...
    while(q--) {
        ...
        if(tp==1) {
            ...
            int j = b / l;
            if(i==j) {
                for(int k=a; k<=b; ++k) ans = min(ans, x[k]);
                cout << ans << endl;
                continue;
            } else {
                //lft
                for(int k=a; k<(i+1)*l; ++k) ans = min(ans, x[k]);
                // mid 
                for(int k=i+1; k<=j-1; ++k) ans = min(ans, blocks[k]);
                //rgt
                for(int k=j*l; k<=b; ++k) ans = min(ans, x[k]);
                cout << ans << endl;
            }
        }
    }
