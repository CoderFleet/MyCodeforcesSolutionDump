...
void mrk(int a, unordered_map<int, vi>& mpp, unordered_set<int>& vis, vi& sbs) {
    vis.insert(a);
...
void solve(){
    ...
    vi sec(n, 0);
    sec[0] = 1;
    sort(sbs.begin(), sbs.end(), [&](auto& s1, auto& s2) {
        return s1[2] < s2[2];
    });
    int i = 0;
    while(i<m) {
        ...
        for(; i<m; ++i) {
            if(sbs[i][2] != cr) break;
            mpp[sbs[i][1]].pb(sbs[i][0]);
            mpp[sbs[i][0]].pb(sbs[i][1]);
        }

        for(auto& [a, b]: mpp) {
            if(vis.count(a) || !sec[a]) continue;
            mrk(a, mpp, vis, sec);
        }
    }
