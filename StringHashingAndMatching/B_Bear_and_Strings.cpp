...
void solve(){
    ...
    int n = s.length();
    string tgt = "bear";
    HashInterval alpha(s);
    HashInterval beta(tgt);
    H target = beta.ha.back();
    // cout << target << endl;
    // cout << alpha.hashInterval(5, 9) << endl;
    // cout << alpha.hashInterval(0, 4) << endl;
    int ans = 0;
    int prev = -1;
    for(int i=0; i+3<n; ++i) {
        if(target == alpha.hashInterval(i, i+4)) {
            // cout << i << endl;
            ans += (i - prev) * (n - i - 3);
            prev = i;
        }
