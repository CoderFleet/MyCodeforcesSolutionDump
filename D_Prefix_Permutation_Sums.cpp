...
void solve(){
    ...
    //     }
    //     mpp[a[i] - a[i-1]]++;
    //     if(a[i] - a[i-1] > n) cul = a[i] - a[i-1];
    //     else if(mpp[a[i] - a[i-1]] > 1) cul = a[i] - a[i-1];
    // }
    for(int i=1; i<n-1; ++i) {
        int diff = a[i] - a[i-1];
        if(diff <= 0) {
        ...
        if(diff > n || mpp[diff] > 0) bad.pb(diff);
        else mpp[diff]++;
    }
    vi msng;
    for(int i=1; i<=n; ++i) {
    ...
    // if(accumulate(all(msng), 0LL) != cul) {
    //     cout << "NO" << endl;
    // } else cout << "YES" << endl;
    // cout << cul << endl;

    if(bad.empty()) {
        cout << (msng.size() == 1 ? "YES" : "NO");
        cout << endl;
    }
    else if(bad.size() == 1 && msng.size() == 2 &&
            bad[0] == msng[0] + msng[1]) {
        cout << "YES";
            nl;
    }
