...
int recurFind(int ind, int curr, int rem, vi& w, int& a, int& b) {
    if(ind < 0) {
        if(curr >= a && curr <= b && rem == 0) return 1;
        else return 0;
    }

    // if(st.find(curr) != st.end()) return dp[curr][ind][rem];
    // st.insert(curr);
    // vvi temp = 
    // dp[curr]
    int np = recurFind(ind-1, curr, rem, w, a, b);
    int p=0;
    if(rem > 0 && (curr + w[ind] <= b)) p = recurFind(ind-1, curr+w[ind], rem-1, w, a, b);
    return p + np;
}
...
void solve(){
    ...
    srt(w);
    int a, b; cin>> a >> b;
    // unordered_map<int, vvi> dp;
    // unordered_set<int> ver;
    int ans = 0;
    ans = recurFind(n-1,0, k, w, a, b);
