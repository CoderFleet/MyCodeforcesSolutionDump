...
string ts;
int dp[15][4096][30][2];
// int dp[15][4096][30][2][2];
int rec(int idx, int mask, int rem, bool tgt, bool ifz) {
    if(idx == ts.size()) {
        return ifz ? 0 : 1;
    }
    if(dp[idx][mask][rem][tgt] != -1 && !tgt) {
        return dp[idx][mask][rem][tgt];
    }

    int res = 0;
    int lim = tgt ?(ts[idx] - '0') : 9;

    for(int d = 0; d <= lim; d++) {
        bool nxtgt = tgt &&(d == lim);
        
        if(ifz && d == 0) {
            res += rec(idx + 1, mask, 0, nxtgt, true);
        } else {
            int nxrem =(rem + d) % k;
            if(!(mask &(1 << nxrem))) {
                res += rec(idx + 1, mask |(1 << nxrem), nxrem, nxtgt, false);
            }

            // int current_mask = ifz ? (mask | (1 << )) : mask; 
            
            // if(!(current_mask & (1 << nxrem))) {
            //     res += rec(idx + 1, current_mask | (1 << nxrem), nxrem, nxtgt, false);
            // }
        }
    }

    if (!tgt) {
        dp[idx][mask][rem][tgt] = res;
    }
    return res;
}

int f(const string& tsn) {
    // int n = tsn.size();
    // if (n <= 0) return 0;
    ts = tsn;
    memset(dp, -1, sizeof(dp));
    return rec(0, 0, 0, true, true);
}
