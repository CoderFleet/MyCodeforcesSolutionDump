...
void solve()
{
    ...
    vi s(n);
    cin >> s;
    // dp[x][j] => maximum number of pages i can buy in x money by choosing books from j to n-1;
    // Transition => dp[x][j] = max(dp[x][j+1], s[j] + dp[x- h[j]][j+1]);
    // vvi dp(x + 1, vi(n, 0));
    vi prev(x+1, 0);
    for (int j = n - 1; j >= 0; --j)
    {
        ...
        rep(i, 1, x)
        {
            curr[i] = prev[i];
            if (h[j] <= i)
                curr[i] = max(curr[i], prev[i - h[j]] + s[j]);
        }
        prev = curr;
    }
