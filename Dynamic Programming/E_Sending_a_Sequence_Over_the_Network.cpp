...
void solve(){
    ...
    // for(int i=n; i>=1; --i) {
    //     if(b[i-1] <= (n - i)) dp1[i] = dp1[i + b[i-1] + 1]; 
    // }
    // // cout << dp1 << endl;
    // if(dp1[1]) {
    //     cout << "YES" << endl;
    ...
    // for(int i=2; i<=n; ++i) {
    //     if(b[i-1] <= i-1) dp2[i] = dp2[i - b[i-1] - 1];
    // }
    ...
    // // cout << dp2 << endl;
    // for(int i=1; i<=n; ++i) {
    //     // if((i+1 <= n+1) && (i+2 <= n+1) && dp2[i+1] && dp1[i+2]) {
    //     //     cout << "YES" << endl;
    //     //     return;
    //     // }
    //     if(dp2[i] && dp1[i+1]) {
    ...
    dp[0] = true;
    for(int i=1; i<=n; ++i) {
        if(b[i-1] + 1 <= i && dp[i - b[i-1] - 1]) dp[i] = true;
        if(b[i-1] + i <= n && dp[i-1]) dp[i + b[i-1]] = true;
    }
    if(dp[n]) cout << "YES";
    else cout << "NO"; nl;
    return;
