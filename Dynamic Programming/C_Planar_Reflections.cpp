...
int helper(int i, int rem, int dir, vector<vvi>& dp, int n)  {
    // if(i < 0 || i >= n) return 0;
    if(rem <= 1) return 0;
    if(dp[i][rem][dir] != -1) return dp[i][rem][dir];

    int v1 = 0, v2 = 0;
    if(dir) {
        v1 = (i-1 >= 0 ?  helper(i-1, rem, 1, dp, n) : 0);
        v2 = 1 + (i+1 < n ? helper(i+1, rem-1, 0, dp, n) : 0);
    } else {
        v1 = (i+1 < n ? helper(i+1, rem, 0, dp, n) : 0);
        v2 = 1 + (i-1 >= 0 ? helper(i-1, rem-1, 1, dp, n) : 0);
    }

    // modadd(v1, v2);

    return dp[i][rem][dir] = (v1 + v2) % MOD;
}

void solve(){
    int n, k; cin >> n >>k;

    // dp[i][j][k] => denotes number of new rays produced by ray striking jth mirror having energy i going in kth direction
    vector<vvi> dp(n+1, vvi(k+1, vi(2, -1)));
    int ans = (helper(0, k, 0, dp, n) + 1) % MOD;
    cout << ans % MOD << endl;
}
