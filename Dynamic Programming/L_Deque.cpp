...
void solve(){
    ...
    for(int l=1; l<n; ++l) {
        for(int j=0; j+l<n; ++j) {
            int a = (j+1 <= j+l-1) ? dp[j+1][j+l-1] : 0;
            int b = (j+2 <= j+l) ? dp[j+2][j+l] : 0;
            int x = (j <= j+l-2) ? dp[j][j+l-2] : 0;
            int y = (j+1 <= j+l-1) ? dp[j+1][j+l-1] : 0;

            dp[j][j+l] = max(arr[j] + min(a, b), arr[j+l] + min(x, y));
        }
    }

    cout << (2 * dp[0][n-1]) - tot << endl;
}
