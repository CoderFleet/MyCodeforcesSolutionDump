...
void solve(){
    ...
    // dp[i][j] => maximum sum that can be obtained from i to j given optimal play from both sides;
    // Transition => dp[i][j] = max(a[i] + min(dp[i+1][j-1], dp[i+2][j]), a[j] + min(dp[i+1][j-1], dp[i][j-2]));
