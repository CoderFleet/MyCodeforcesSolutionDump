...
int n, m;
int dp[10][1001][1001];
int recurCount(int ind, int a, int b) {
    if(ind == m) return 1;
    if(a > b) return 0;
    if(a > n) return 0;
    if(b < 1) return 0;
    ...
    return dp[ind][a][b] = (recurCount(ind, a+1, b) + recurCount(ind+1, a, b) + recurCount(ind, a, b-1) - recurCount(ind, a+1, b-1) + MOD) % MOD;
}
void solve(){
    cin >> n >> m;
    ...
    cout << recurCount(0, 1, n) % MOD << endl;
}
