...
// int recurCount(int x, vi& c, vi& dp) {
//     if(x < 0) return 0;
//     if(x == 0) return 1;
...
//     fr(i, c.size()) {
//         if(c[i] <= x)
//         modadd(wys, recurCount(x - c[i], c, dp));
//         else break;
//     }
...
void solve(){
    ...
    // fr(i, n) if(c[i] <= x)dp[c[i]] = 1;

    // dp[i] means number of ways to produce sum i with these given coin types
    rep(i, 1, x) {
        rep(j, 0, n-1) if(i - c[j] >= 0) dp[i] = (dp[i] + dp[i - c[j]])%MOD;
    }
