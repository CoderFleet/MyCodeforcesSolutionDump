...
void solve(){
    ...
    // dp[i][j] => Number of ways generate sum 'j' knowing coins from i to n-1 are still pickable while coins from 0 to i-1 can't be picked now

    for(int i=n-1; i>=0; --i) {
        // vi curr(x+1, 0);
        // curr[0] = 1;
        for(int j=c[i]; j<=x; ++j) {
            // prev[j] = (prev[j] + prev[j - c[i]]);
            prev[j] = (prev[j] + prev[j - c[i]])%MOD;
        }
        // prev = curr;
    }
