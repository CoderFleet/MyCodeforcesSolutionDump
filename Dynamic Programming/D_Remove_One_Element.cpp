...
void solve(){
    ...
    // dp[i][0] denotes longest subarray till ith element without removing it;
    // dp[i][1] denotes longest subarray till ith element after removing it;
    int ans = 1;
    ...
    for(int i=1; i<=n-2; ++i) {
        if(a[i-1] < a[i+1]) ans = max(ans, rg[i-1]+lf[i+1]);
    }
